...
const CertificateGallery = (props) =>{
  ...
  const [images, setImages] = React.useState(artistPayload.certificates ? artistPayload.certificates : []);
  ...
  const handleUpload = async (files) => {
    ...
    try {
      ...
      setProgress(0)
      setUploading(false)
      setImages(response?.data?.data?.certificates)
  ...
  const updateImages = async (certificates) =>{
    
    try {
      const response = await axiosAuth.post(`${BASE_URL}/users/updateArtistRequest`, {certificates});
