...
const Certificates = () => {
  ...
  const [certificates,setCertificates] = useState(artistPayload.certificates ? artistPayload.certificates : [])
  ...
  const removeDocument = async(index) =>{
    ...
    try{
    const response = await axiosAuth.post(`${BASE_URL}/users/updateArtistRequest`, {'certificates':updatedCertificates});setArtistPayload(response?.data?.data);
    setCertificates(response?.data?.data?.certificates)
    // Handle response if needed
  } catch (error) {
    console.error(error, 'file upload error');
    ...
    try{
      if(artistPayload.currentStep > 13){
       return  navigate(`/become-a-artist/${request_id}/personal-details`)
        ...
        setArtistPayload((prev) => {return {...prev,currentStep:14}})
      ...
      <section>
        ...
        <div className="customized-gallery pt-4">
          ...
          <div className="row gallery-row g-3">
            <>
            {(certificates && certificates.length > 0) || (binaryFiles && binaryFiles.length > 0) ?
            <>
            {
              certificates.map((item,index)=>{
                return (<div className='col-md-6 p-3'>
                  <div className='custom-kyc-img-wrapper'>
                    ...
                    <>
                      <img src={PdfIcon} alt={item.name} className="img-fluid w-100"/>
                    <div className='custom-kyc-update-dropshadow-box'>
                      <div>
                      <MdDeleteForever onClick={()=>removeDocument(index)} className="me-3"/>
                    ...
                    :
                    <>
                    <img src={item.url} alt={item.name} className='img-fluid w-100'/>
                  <div className='custom-kyc-update-dropshadow-box' onClick={()=>removeDocument(index)}>
                  <MdDeleteForever />
                  </div>
                  </>
                    }
            ...
            }
            {
              binaryFiles.map((item)=>{
                let src= item.type == 'application/pdf' ? PdfIcon : URL.createObjectURL(item)
                return (<div className='col-md-6 p-3'>
                  <div className='custom-kyc-img-wrapper'>
                      <img src={src} alt="binary File" className='img-fluid w-100'/>
            ...
            <div {...getRootProps({className: 'custom-add-more-files-card'})}>
            <input {...getInputProps()} />
